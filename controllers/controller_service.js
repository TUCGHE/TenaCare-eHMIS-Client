...
'use strict';
app.controller('servicedataentryController', function ($scope, servicedataentryService, ValueService, LanguageFactory) {   
    ...
    $scope.getLanguageLabel = function (lang, className) {
		...
		.then(function (data) {
		    ...
		    $scope.title = $scope.dataEntry['monthlyServiceDataEntry'];
    ...
    $scope.ChangedValue = [];
    var chagedData = [];
    $scope.getChangedValue = function (year, month, value, lbid,LocationId,facilityTypeId,zone,region,woreda) {
    ...
    //year, quarter,week,month, dataEleClass , LocationID
    $scope.onYearSelect = function (LocationID) {       
        var monthIndex = getMonthIndex($scope.selectedMonth);
        ValueService.getHmisData($scope.selectedYear, 0, 0, monthIndex, 6, LocationID, function (response) {
            $scope.loadedData = response.data;
            //this.fetch(10);
        });
    }
    $scope.onQuarterSelect = function (LocationID) {
        var monthIndex = getMonthIndex($scope.selectedMonth);
        ValueService.getHmisData($scope.selectedYear, 0, 0, monthIndex, 6, LocationID, function (response) {
            $scope.loadedData = response.data;
            //this.fetch(10);
    ...
    $scope.loadData = function (locationId) {
        var monthIndex = getMonthIndex($scope.selectedMonth);
        ValueService.getHmisData($scope.selectedYear, 0, 0, monthIndex, 6, temphierarchy[0].HMISCode, function (response) {
    ...
    $scope.$watch('temphierarchy[0].HMISCode', function (newValue, oldValue) {
        if (newValue != undefined || newValue != oldValue) {
            fetch(temphierarchy[0].FacilityTypeId);
            $scope.loadData($scope.LocationId);
        }
    });
    ...
    var fetch = function (facilityType) {
        ...
        var limit = 0;
        servicedataentryService.get(limit, FacilityType, 0, function (response) {
            ...
            $scope.status = $scope.dataEntry['failedToLoadTheValues'];
        });      
    }
